...
import { STUDENTS_BY_QUERY } from "@/sanity/lib/queries";
...
import { columns } from "@/components/admin/students/column";
...
export default function StudentsTable() {
  ...
  const handleEdit = async (request: DataProps) => {
    ...
    router.push(`/admin/ho-so-hoc-vien/${request._id}`);
  };
  ...
  const getStudents = async () => {
    ...
    console.log("getStudents -> searchForProjects", searchForProjects);
    setStudents(searchForProjects);
  };
  ...
  const _columns: ColumnDef<Student>[] = [
    ...columns,
    {
      id: "actions",
      header: "Action",
      ...
      <section className="w-full bg-white rounded-2xl p-7">
        <div className="flex items-center justify-end px-6">
          <Button
            ...
            asChild
          >
            <Link href="/admin/ho-so-hoc-vien/new">
              <span className="flex items-center">
                <span className="mr-1">+</span> Tạo học viên mới
              </span>
            </Link>
          </Button>
        </div>
