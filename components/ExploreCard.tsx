...
const ExploreCard = (
  { id, imgUrl, title, index, active, handleClick }:
    { id: string, imgUrl: string, title: string, index: number, active: string, handleClick(id: string): void }
) => (
  <motion.div
    variants={fadeIn('right', 'spring', index * 0.5, 0.75)}
    className={`relative ${active === id
      ? 'lg:flex-[3.5] flex-[10]'
      : 'lg:flex-[0.5] flex-[2] flex items-center justify-center min-w-[170px] h-[700px] transition-[flex] duration-700 ease-out-flex cursor-pointer'
      }`}
