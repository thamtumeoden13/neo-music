    ...
    timeoutRef.current = setTimeout(() => {
      setBeams(
        Array.from({ length: beamCount }, (_, index) => {
          const translateX = Math.floor(Math.random() * pageWidth);
          return {
            initialX: translateX,
            translateX: translateX,
            initialY: pageScroll - 100,
            translateY: pageHeight,
            duration: Math.floor(Math.random() * 10) + 3,
            delay: Math.floor(Math.random() * 5),
            className: `h-${Math.floor(Math.random() * 20)}`,
          }
        })
      );
    }, 2000);
  ...
  return (
    <>
      <motion.div
        ...
        variants={{
          animate: {
            translateY: beamOptions.translateY ? beamOptions.translateY : "1800px",
            translateX: beamOptions.translateX ? beamOptions.translateX : "0px",
            rotate: beamOptions.rotate ?? 0,
          },
        }}
        transition={{
        ...
        className={cn(
          "absolute left-0 top-20 m-auto h-14 w-px rounded-full bg-gradient-to-t from-[#65c4fa] via-[#34acc7] to-transparent",
